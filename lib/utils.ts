...
export const getMediaStreams = async (withMic: boolean): Promise<MediaStreams> => {
        ...
        if (withMic) {
                ...
                micStream.getAudioTracks().forEach((track: MediaStreamTrack) => (track.enabled = true));
        }
...
export const getVideoDuration = (url: string): Promise<number | null> =>
        new Promise((resolve) => {
                ...
                video.onloadedmetadata = () => {
                        const duration =
                                isFinite(video.duration) && video.duration > 0 ? Math.round(video.duration) : null;
...
export const createRecordingBlob = (chunks: Blob[]): { blob: Blob; url: string } => {
...
export const createIframeLink = (videoId: string) =>
        `https://iframe.mediadelivery.net/embed/476908/${videoId}?autoplay=true&preload=true`;
